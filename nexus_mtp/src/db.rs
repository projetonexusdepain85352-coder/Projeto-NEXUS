...
pub async fn fetch_approved_documents(
    pool: &PgPool, domain: &str, limit: i64,
...
pub async fn create_training_cycle(
    pool: &PgPool, domain: &str, base_model: &str,
    config: &JsonValue, dataset_size: i32,
) -> Result<Uuid> {
    ...
    .bind(domain).bind(base_model).bind(config).bind(dataset_size)
    .fetch_one(pool).await?;
    Ok(row.0)
}

pub async fn complete_training_cycle(pool: &PgPool, cycle_id: Uuid, final_loss: Option<f32>) -> Result<()> {
    sqlx::query(
...
pub async fn fail_training_cycle(pool: &PgPool, cycle_id: Uuid) -> Result<()> {
    sqlx::query("UPDATE training_cycles SET status = 'failed', completed_at = NOW() WHERE id = $1")
        .bind(cycle_id).execute(pool).await?;
    Ok(())
}

pub async fn create_model(
    pool: &PgPool, name: &str, domain: &str, base_model: &str,
    dataset_size: i32, training_steps: i32, adapter_path: &str,
    adapter_checksum: &str, cycle_id: Uuid,
) -> Result<Uuid> {
    let row: (Uuid,) = sqlx::query_as(
        "INSERT INTO models
    ...
    .bind(name).bind(domain).bind(base_model).bind(dataset_size)
    .bind(training_steps).bind(adapter_path).bind(adapter_checksum).bind(cycle_id)
    .fetch_one(pool).await?;
    Ok(row.0)
}
...
pub async fn get_model(pool: &PgPool, model_id: Uuid) -> Result<ModelRow> {
    sqlx::query_as::<_, ModelRow>("SELECT * FROM models WHERE id = $1")
        .bind(model_id).fetch_optional(pool).await?
...
pub async fn update_benchmark_score(pool: &PgPool, model_id: Uuid, score: f32) -> Result<()> {
    sqlx::query("UPDATE models SET benchmark_score = $2 WHERE id = $1")
        .bind(model_id).bind(score).execute(pool).await?;
    Ok(())
}
...
pub async fn insert_lineage(pool: &PgPool, doc_ids: &[Uuid], cycle_id: Uuid) -> Result<()> {
    for doc_id in doc_ids {
        ...
        .bind(doc_id).bind(cycle_id).execute(pool).await?;
    }
    Ok(())
}
...
pub async fn active_model_per_domain(pool: &PgPool) -> Result<Vec<(String, Option<String>)>> {
    Ok(sqlx::query_as(
        "SELECT domain, name FROM models WHERE status = 'deployed' ORDER BY domain",
    ).fetch_all(pool).await?)
}

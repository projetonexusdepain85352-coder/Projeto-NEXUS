...
Este documento registra erros, tentativas falhas e solucoes confirmadas durante a execucao do benchmark do `nexus_mtp`.
...
O benchmark original dependia de Candle para inferencia, mas o modelo base e os adapters foram treinados com BitsAndBytes 4-bit. Essa combinacao e incompativel e leva a erro de shape no carregamento.
...
- [ ] Adapter path no banco e relativo (ex.: `adapters/<model>`).

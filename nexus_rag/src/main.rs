...
use qdrant_client::qdrant::{
    PayloadIncludeSelector, PointId, ScrollPointsBuilder,
    value::Kind,
};
use tokio::signal;
...
#[derive(Parser, Debug)]
#[command(name = "nexus_rag", version = "0.1.0", about = "NEXUS private AI — RAG component")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
enum Commands {
    ...
    Query {
        ...
        #[arg(long, default_value_t = 5)] top: usize,
    },
...
async fn run_status() -> Result<()> {
    ...
    for desc in all_collections.collections {
        if !desc.name.starts_with("nexus_") { continue; }
        let info = client.collection_info(&desc.name).await.map_err(error::qdrant_err)?;
        let pts  = info.result.as_ref().and_then(|r| r.points_count).unwrap_or(0);
    ...
    for coll_name in qdrant_points.keys() {
        let domain_key = coll_name.strip_prefix("nexus_").unwrap_or(coll_name).to_string();
    ...
    println!("    {:30}  {:>10}  {:>10}  {:>12}", "Domain", "Approved", "Indexed", "Delta");
    println!("    {}", "─".repeat(68));
    ...
    for domain in &all_domains {
        ...
        ga += approved; gi += indexed; gd += delta;
        ...
        println!("    {:30}  {:>10}  {:>10}  {}", domain, approved, indexed, delta_disp);
    }

    println!("    {}", "─".repeat(68));
    let grand_disp = if gd > 0 { format!("⚠  {:>+8}", gd) } else { format!("✓  {:>8}", 0) };
    println!("    {:30}  {:>10}  {:>10}  {}", "TOTAL", ga, gi, grand_disp);
    println!();
    if gd > 0 {
        println!("  ⚠  {} document(s) not yet indexed. Run `nexus_rag index`.", gd);
    } else {

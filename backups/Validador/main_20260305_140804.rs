...
fn buscar_lote(client: &mut Client, pulados_sessao: &HashSet<String>) -> Vec<Documento> {
    let excluidos: Vec<&str> = pulados_sessao.iter().map(|s| s.as_str()).collect();

    // Busca 4 000 chars brutos para que o filtro tenha material suficiente
    // mesmo em documentos com muito conteúdo de navegação no início.
    let sql_base = "
        SELECT d.id::text, d.source, d.domain, d.doc_type, d.content_length,
               LEFT(convert_from(convert_to(d.content, 'UTF8'), 'UTF8'), 4000) AS raw_content,
               RIGHT(convert_from(convert_to(d.content, 'UTF8'), 'UTF8'), 1000) AS raw_tail
        FROM documents d
        JOIN validation v ON v.document_id = d.id
        WHERE v.status = 'pending'
    ";
...
fn exibir_documento(
    ...
    println!("  [i] Sugestão [h] Heurística  [t] Auto-IA  [x] Parar-IA  [v] Voltar  [?] Ver mais  [e] Stats");
    println!("──────────────────────────────────────────────────────────────");
}
...
fn validar_resposta(_dominio: &str, motivo: &str, confianca: u8) -> bool {
    if confianca < CONFIANCA_MINIMA { return false; }
    if motivo.len() < 10 { return false; }
    true
}

fn chamar_ollama(dominio: &str, conteudo: &str, tentativa: u8) -> Option<Sugestao> {
...
fn sugerir_ia(doc: &Documento) -> Option<Sugestao> {
    for tentativa in 1u8..=2 {
        if let Some(s) = chamar_ollama(&doc.domain, &doc.content, tentativa) {
            if validar_resposta(&doc.domain, &s.motivo, s.confianca) {
                return Some(s);
            }
            if tentativa == 2 {
...
fn sugerir_ia_thread(dominio: &str, conteudo: &str) -> Option<Sugestao> {
    for tentativa in 1u8..=2 {
        if let Some(s) = chamar_ollama(dominio, conteudo, tentativa) {
            if validar_resposta(dominio, &s.motivo, s.confianca) {
                return Some(s);
            }
            if tentativa == 2 {
                return Some(Sugestao {
...
fn sugerir(doc: &Documento) -> Sugestao {
    if let Some(s) = sugerir_ia(doc) { return s; }
    sugerir_heuristica_interna(doc)
}
...
fn sugerir_heuristica_interna(doc: &Documento) -> Sugestao {
    ...
    let sinal_toc_ativo = pct_lista > 40 || pct_toc_inline > 20;
...
fn main() {
    ...
    // Macro local para atualizar o estado no disco após cada decisão
    macro_rules! persistir_estado {
        ($ultimo_id:expr) => {
            salvar_estado_sessao(&EstadoSessao {
                ...
                ultimo_documento_id: Some($ultimo_id.to_string()),
            });
        };
    }
        ...
        while idx < total_lote {
            ...
            match &modo_sugestao {
                ...
                ModoSugestao::IA => {
                    ...
                    let cont_clone    = doc.content.clone();
                    let parar_clone   = Arc::clone(&parar_auto);
                    let handle = std::thread::spawn(move || {
                        sugerir_ia_thread(&dom_clone, &cont_clone)
                    });
                    ...
                    if parar_auto.load(Ordering::Relaxed) {
                        ...
                        println!("  [IA] Auto-IA interrompida.");

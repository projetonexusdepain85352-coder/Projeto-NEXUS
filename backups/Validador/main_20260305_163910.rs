...
impl ModoSugestao {
    fn proximo(&self) -> ModoSugestao {
        match self {
            ModoSugestao::Desligado  => ModoSugestao::Heuristica,
            ModoSugestao::Heuristica => ModoSugestao::IA,
            ModoSugestao::IA         => ModoSugestao::Desligado,
        }
    }
...
fn revalidar_ia(client: &mut Client, config: &Config, stdin: &io::Stdin) {
    ...
    let parar_stdin = Arc::clone(&parar);
    std::thread::spawn(move || {
        let si = io::stdin();
        let mut b = String::new();
        si.lock().read_line(&mut b).ok();
        if b.trim() == "x" { parar_stdin.store(true, Ordering::Relaxed); }
    });
    ...
    for (i, row) in rows.iter().enumerate() {
        ...
        let handle = std::thread::spawn(move || {
            sugerir_ia(&dom_clone, &prev_clone, timeout_s)
        });
    ...
    if !pulados_retry.is_empty() && !parar.load(Ordering::Relaxed) {
        ...
        io::stdout().flush().ok();
        let mut buf_retry = String::new();
        stdin.lock().read_line(&mut buf_retry).ok();

        if buf_retry.trim().to_lowercase() == "s" {
            let parar_retry = Arc::new(AtomicBool::new(false));
            ...
            // ── primeira passagem: processa cada doc com IA ───────────────
            for (ri, (r_id, r_source, r_domain, r_raw)) in pulados_retry.iter().enumerate() {
                if parar_retry.load(Ordering::Relaxed) { break; }
                ...
                // Thread stdin para capturar x durante os dots — igual ao loop principal
                let parar_x = Arc::clone(&parar_retry);
                let stdin_handle = std::thread::spawn(move || {
                    let mut b = String::new();
                    io::stdin().lock().read_line(&mut b).ok();
                    if b.trim() == "x" { parar_x.store(true, Ordering::Relaxed); }
                });
                let mut dots = 0usize;
                loop {
                    if handle.is_finished() || parar_retry.load(Ordering::Relaxed) { break; }
                    std::thread::sleep(std::time::Duration::from_millis(300));
                    print!("."); io::stdout().flush().ok();
                    dots += 1;
                    if dots > max_dots { break; }
                }
                // Descarta stdin_handle — igual ao loop principal
                drop(stdin_handle);
                if parar_retry.load(Ordering::Relaxed) {
                    println!();
                    println!("  Interrompido.");
                    break;
                }
                match handle.join().ok().flatten() {
                    // Ainda sem resposta -> fila_manual direto
                    None => {
                        println!(" sem resposta (retry)");
                        fila_manual.push((r_id.clone(), r_source.clone(), r_domain.clone(), r_raw.clone()));
                    }
                    // IA respondeu mas heurística insegura -> aguarda 30s usuário
                    Some(s) if heur.confianca < config.threshold_heuristica => {
                        println!(
                            " IA {}% ({}) | Heur {}%",
                            s.confianca,
                            if s.categoria == Categoria::Util { "util" } else { "inutil" },
                            heur.confianca
                        );
                        print!("  [a] Aprovar  [r/u] Rejeitar  [x] Parar  (30s): ");
                        io::stdout().flush().ok();
                        let (tx_dec, rx_dec) = std::sync::mpsc::channel::<String>();
                        let parar_dec = Arc::clone(&parar_retry);
                        std::thread::spawn(move || {
                            let mut b = String::new();
                            io::stdin().lock().read_line(&mut b).ok();
                            let v = b.trim().to_lowercase();
                            if v == "x" { parar_dec.store(true, Ordering::Relaxed); }
                            tx_dec.send(v).ok();
                        });
                        match rx_dec.recv_timeout(std::time::Duration::from_secs(30))
                            .unwrap_or_default().as_str()
                        {
                            "x" => { println!("  -> Interrompido."); }
                            "a" => {
                            ...
                            _ => {
                                // Timeout 30s -> fila_manual
                                println!("  -> Sem resposta — voltara depois.");
                                fila_manual.push((r_id.clone(), r_source.clone(), r_domain.clone(), r_raw.clone()));
                            }
                        }
                    }
                    // Confiança suficiente -> decisão automática
                    Some(s) if s.confianca >= config.threshold_ia => {
                        let util      = s.categoria == Categoria::Util;
                        let motivo_ia = format!("[ia-rev] {}", s.motivo);
                    ...
                    // Confiança baixa mas heur ok -> mantido
                    Some(s) => {
                        println!(" confianca baixa ({}%) — mantido", s.confianca);
                        n_incerto += 1;
                    }
            ...
            // ── loop manual: exibe pendentes em rodadas de 30s por doc ────
            while !fila_manual.is_empty() && !parar_retry.load(Ordering::Relaxed) {
                println!();
                ...
                for (fi, (f_id, f_source, f_domain, f_raw)) in fila_manual.iter().enumerate() {
                    ...
                    let parar_dec2 = Arc::clone(&parar_retry);
                    std::thread::spawn(move || {
                        let mut b = String::new();
                        io::stdin().lock().read_line(&mut b).ok();
                        let v = b.trim().to_lowercase();
                        if v == "x" { parar_dec2.store(true, Ordering::Relaxed); }
                        tx2.send(v).ok();
                    });

                    match rx2.recv_timeout(std::time::Duration::from_secs(30))
                        .unwrap_or_default().as_str()
                    {
                        "x" => {
                            println!("  -> Interrompido.");
                        ...
                        _ => {
                            // Timeout -> permanece na fila para próxima rodada
                            println!("  -> Sem resposta — proxima rodada.");
                        }
                    }
                }

                // Remove respondidos (índices em ordem reversa para não deslocar)
...
fn config_tui(config: &mut Config, client: &mut Client, stdin: &io::Stdin) {
    loop {
        ...
        println!();
        println!("  [6] Revalidar tags IA    : atualiza tags de todos os docs validados por IA");
        println!();
...
fn exibir_ajuda() {
    ...
    println!("  │  [h] Heurística (liga/desliga sugestão heurística) │
  │  [t] Auto-IA   (liga/desliga IA automática)         │");
...
fn palavras_dominio(dominio: &str) -> &'static [&'static str] {
    match dominio {
        "rust"     => &["rust", "cargo", "trait", "lifetime", "borrow", "ownership",
                        "unsafe", "async", "macro", "enum", "struct", "closure", "iterator"],
        "infra"    => &["kernel", "docker", "syscall", "driver", "systemd", "postgres",
                        "container", "namespace", "cgroup", "mount", "socket", "processo",
                        "scsi", "dma", "irq", "firmware", "subsystem", "hardware", "device",
                        "bus", "memory", "interrupt", "scheduler", "filesystem", "network",
                        "protocol", "armazenamento", "dispositivo", "redes", "sistema",
                        "tty", "uart", "usb", "pci", "ioctl", "mmap", "buffer", "cache",
                        "mutex", "spinlock", "semaphore", "thread", "process", "inode",
                        "pagefault", "vmalloc", "kmalloc", "slab", "hugepage", "tlb"],
        "security" => &["vulnerabilidade", "cve", "exploit", "ataque", "criptografia",
                        "autenticacao", "tls", "owasp", "injecao", "buffer", "overflow"],
        "mlops"    => &["treinamento", "fine-tuning", "qlora", "lora", "transformer",
                        "embedding", "quantizacao", "inferencia", "dataset", "gradiente"],
        _          => &[],
    }
}
...
fn sugerir_heuristica_interna(doc: &Documento) -> Sugestao {
    ...
    for l in &linhas {
        if l.len() > 5 && !vistos.insert(l) { duplicadas += 1; }
    }
        ...
        while idx < total_lote {
            ...
            match &modo_sugestao {
                ...
                ModoSugestao::IA => {
                    ...
                    let parar_clone   = Arc::clone(&parar_auto);
                        ...
                        if heur.confianca < config.threshold_heuristica {
                            ...
                            match decisao_manual.as_str() {
                                ...
                                "r" | "u" => {
                                    ...
                                    tags_auto.push(format!("heur:{}", heur.confianca));
                                    db_salvar_tags(&mut client, &doc.id, &tags_auto);
                                    historico.push(HistoricoItem { id: doc.id.clone(), acao: Acao::Rejeitado(motivo) });
                                    rejeitados += 1;
                                    persistir!(&doc.id);
                                    idx += 1;
                        ...
                        if s.confianca >= config.threshold_ia {
                            ...
                            match s.categoria {
                                ...
                                Categoria::Inutil => {
                                    ...
                                    tags_auto.push(format!("heur:{}", heur.confianca));
                                    db_salvar_tags(&mut client, &doc.id, &tags_auto);
                                    historico.push(HistoricoItem { id: doc.id.clone(), acao: Acao::Rejeitado(motivo_ia) });
                                    rejeitados += 1;
                                    persistir!(&doc.id);
                                    idx += 1;

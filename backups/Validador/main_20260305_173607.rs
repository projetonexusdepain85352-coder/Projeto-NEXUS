...
fn revalidar_ia(client: &mut Client, config: &Config, stdin: &io::Stdin) {
    ...
    for (i, row) in rows.iter().enumerate() {
        ...
        exibir_documento_revalidar(&doc_tmp, i, total as usize);
        ...
        match handle.join().ok().flatten() {
            ...
            Some(s) => {
                ...
                if heur.confianca < config.threshold_heuristica {
                    // Heurística baixa — pula para retry sem pedir input
                    pulados_retry.push((id.clone(), source.clone(), domain.clone(), raw.clone()));
                    n_incerto += 1;
                    std::thread::sleep(std::time::Duration::from_millis(400));
...
fn config_tui(config: &mut Config, client: &mut Client, stdin: &io::Stdin) {
    loop {
        ...
        println!();
        println!("  [6] Revalidar auto IA    : roda Ollama em todos os docs validados por IA");
        println!();
...
fn exibir_documento_revalidar(doc: &Documento, idx: usize, total: usize) {
    limpar_tela();
    let conteudo_header = format!(
...
fn exibir_resultado_revalidar(s: &Sugestao, heur: &Sugestao) {
    ...
    // label fixo em 9 chars: "APROVADO " / "REJEITADO" / "UTIL     " / "INUTIL   "
    ...
    // confiança formatada com 3 chars para alinhar "90%" e " 61%"
    let conf_ia_str   = format!("{:3}%", s.confianca);
    let conf_heur_str = format!("{:3}%", heur.confianca);

    // Caixa de resultado
    // Largura interna fixa: "  IA   : APROVADO  90% [██████████]" = 38 chars + padding + "  │"
    // Linha completa: "  │  " (5) + conteudo (38) + "  │" (3) = 46... usamos padding dinâmico
    let linha_ia   = format!("IA   : {} {} [{}]", label_ia,   conf_ia_str,   barra_ia);
    let linha_heur = format!("Heur : {} {} [{}]", label_heur, conf_heur_str, barra_heur);
    // largura da caixa = 64 chars internos (entre os │)

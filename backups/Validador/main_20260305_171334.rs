...
fn revalidar_ia(client: &mut Client, config: &Config, stdin: &io::Stdin) {
    ...
    for (i, row) in rows.iter().enumerate() {
        ...
        exibir_documento(&doc_tmp, i, total as usize, 0, 0, 0, 0, 30, &ModoSugestao::IA);
        ...
        match handle.join().ok().flatten() {
            ...
            Some(s) => {
                println!();
                exibir_sugestao(&s);
                println!("  Heurística: {}%", heur.confianca);
                ...
                if heur.confianca < config.threshold_heuristica {
                    ...
                    if util {
                        db_aprovar_ia(client, &id);
                        n_aprovados += 1;
                        println!("  [IA] APROVADO automaticamente ({}% | heur:{}%)", s.confianca, heur.confianca);
                    } else {
                        db_rejeitar_ia(client, &id, &motivo_ia);
                        n_rejeitados += 1;
                        println!("  [IA] REJEITADO automaticamente ({}% | heur:{}%)", s.confianca, heur.confianca);
                    }
                    client.execute(
                    ...
                    println!("  [IA] Confiança baixa ({}%) — mantido.", s.confianca);
                    n_incerto += 1;
                    std::thread::sleep(std::time::Duration::from_millis(400));
                }
            }

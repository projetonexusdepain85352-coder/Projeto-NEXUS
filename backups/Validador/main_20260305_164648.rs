...
fn revalidar_ia(client: &mut Client, config: &Config, stdin: &io::Stdin) {
    ...
    if !pulados_retry.is_empty() && !parar.load(Ordering::Relaxed) {
        ...
        if buf_retry.trim().to_lowercase() == "s" {
            ...
            // Thread único de stdin para todo o retry — sem race condition
            let (tx_retry, rx_retry) = std::sync::mpsc::channel::<String>();
            let parar_rx = Arc::clone(&parar_retry);
            std::thread::spawn(move || {
                loop {
                    let mut b = String::new();
                    io::stdin().lock().read_line(&mut b).ok();
                    let v = b.trim().to_lowercase();
                    if v == "x" { parar_rx.store(true, Ordering::Relaxed); }
                    if tx_retry.send(v).is_err() { break; }
                }
            });
            ...
            for (ri, (r_id, r_source, r_domain, r_raw)) in pulados_retry.iter().enumerate() {
                ...
                loop {
                    if handle.is_finished() { break; }
                    if parar_retry.load(Ordering::Relaxed) { break; }
                    // Drena canal: x digitado durante dots
                    while rx_retry.try_recv().is_ok() {}
                    std::thread::sleep(std::time::Duration::from_millis(300));
                    print!("."); io::stdout().flush().ok();
                    dots += 1;
                    if dots > max_dots { break; }
                }
                // Após IA terminar, dá 150ms para o thread processar x digitado no mesmo instante
                ...
                match handle.join().ok().flatten() {
                    ...
                    Some(s) if heur.confianca < config.threshold_heuristica => {
                        // Exibe resultado no mesmo estilo do loop principal
                        println!();
                        ...
                        print!("  [IA] Heuristica baixa ({}%) — sua decisao em 30s [a/r/u] ou pula: ", heur.confianca);
                        io::stdout().flush().ok();
                        match rx_retry.recv_timeout(std::time::Duration::from_secs(30))
            ...
            // ── loop manual: docs que não foram respondidos ───────────────

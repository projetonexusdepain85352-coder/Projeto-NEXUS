        ...
        match handle.join().ok().flatten() {
            ...
            Some(s) => {
                ...
                if heur.confianca < config.threshold_heuristica {
                    print!("  [IA] Heurística baixa — sua decisão em 30s [a/r/u] ou pula: ");
                    io::stdout().flush().ok();
                        ...
                        .unwrap_or_default().as_str()
                    {
                        ...
                        _ => {
                            println!("  -> Sem resposta — voltará depois.");
                            pulados_retry.push((id.clone(), source.clone(), domain.clone(), raw.clone()));
                            n_incerto += 1;
                        }
                    }
                    ...
                    if util {
                        db_aprovar_ia(client, &id);
                        n_aprovados += 1;
                        println!("  [IA] APROVADO automaticamente (ia:{}% | heur:{}%)", s.confianca, heur.confianca);
                    } else {
                        db_rejeitar_ia(client, &id, &motivo_ia);
                        n_rejeitados += 1;
                        println!("  [IA] REJEITADO automaticamente (ia:{}% | heur:{}%)", s.confianca, heur.confianca);
                    }
                    client.execute(
                    ...
                    println!("  [IA] Confiança baixa (ia:{}% | heur:{}%) — mantido.", s.confianca, heur.confianca);
                    n_incerto += 1;
                    std::thread::sleep(std::time::Duration::from_millis(400));
                }
            }

        ...
        match handle.join().ok().flatten() {
            ...
            Some(s) => {
                ...
                if heur.confianca < config.threshold_heuristica {
                    ...
                    loop {
                        print!("  [HEU] Heuristica baixa ({}%) — sua decisao em 30s [a/r/u/? ] ou pula: ", heur.confianca);
                        io::stdout().flush().ok();
...
fn exibir_documento(
    ...
    let n_linhas = if !doc.content.is_empty() { doc.content.lines().count() } else { doc.preview.lines().count() };

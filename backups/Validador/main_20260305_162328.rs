        ...
        match handle.join().ok().flatten() {
            ...
            Some(s) if s.confianca >= config.threshold_ia => {
                ...
                if util {
                    ...
                    n_aprovados += 1;
                    println!(" APROVADO ({}%)", s.confianca);
                } else {
                    db_rejeitar_ia(client, &id, &motivo_ia);
                    n_rejeitados += 1;
                    println!(" REJEITADO ({}%)", s.confianca);
                }
                client.execute(
    ...
    if !pulados_retry.is_empty() && !parar.load(Ordering::Relaxed) {
        ...
        if buf_retry.trim().to_lowercase() == "s" {
            ...
            for (ri, (r_id, r_source, r_domain, r_raw)) in pulados_retry.iter().enumerate() {
                ...
                // Canal único por doc: serve para x nos dots E para decisão manual
                ...
                std::thread::spawn(move || {
                    ...
                    if v == "x" { parar_x.store(true, Ordering::Relaxed); }
                    tx_stdin.send(v).ok();
                });
                ...
                loop {
                    ...
                    if rx_stdin.try_recv().is_ok() { break; }
                    std::thread::sleep(std::time::Duration::from_millis(300));
                    print!("."); io::stdout().flush().ok();
                    dots += 1;
                    if dots > max_dots { break; }
                ...
                // Drena canal: x pode ter chegado enquanto IA terminava
                if let Ok(v) = rx_stdin.try_recv() {
                    if v == "x" { parar_retry.store(true, Ordering::Relaxed); }
                }

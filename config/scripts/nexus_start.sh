...
CLOUDFLARED="$ROOT_DIR/src/nexus_control_server/bin/cloudflared"
...
pkill -f "src/nexus_control_server/server.py" 2>/dev/null; sleep 1
cd "$ROOT_DIR"
...
curl -s "https://api.cloudflare.com/client/v4/accounts/${CF_ACCOUNT_ID}/workers/scripts/${CF_WORKER_NAME}" \
  ...
  --data-binary "$WORKER_SCRIPT" | python3 -c "import sys,json; r=json.load(sys.stdin); print('[3/3] âœ… Worker atualizado!' if r.get('success') else f'âŒ Erro: {r}')"

echo ""
echo "â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•"
echo "  NEXUS ONLINE âœ…"
echo "  URL FIXA: $CF_WORKER_URL"
echo "  (Google OAuth configurado uma vez sÃ³)"
echo "â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•"

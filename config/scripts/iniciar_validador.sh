...
PROJECT_ROOT="${NEXUS_PROJECT_ROOT:-$(cd "$SCRIPT_DIR/../.." && pwd)}"

if [[ -f "$PROJECT_ROOT/config/scripts/run_copy_env.sh" ]]; then
    # shellcheck source=/dev/null
    source "$PROJECT_ROOT/config/scripts/run_copy_env.sh" >/dev/null
fi

if [[ -f "$PROJECT_ROOT/config/scripts/ensure_permissions.sh" ]]; then
    # shellcheck source=/dev/null
    source "$PROJECT_ROOT/config/scripts/ensure_permissions.sh"
fi
...
if ! declare -F nexus_pg_check_reader >/dev/null || ! declare -F nexus_pg_reapply_reader_grants >/dev/null; then
    echo "[ERRO] Funcoes de permissao nao encontradas (config/scripts/ensure_permissions.sh)."
    exit 1
fi
...
python3 "$PROJECT_ROOT/src/nexus_sugestor/servidor.py" > "$PROJECT_ROOT/src/nexus_sugestor/servidor.log" 2>&1 &
SERVIDOR_PID=$!
